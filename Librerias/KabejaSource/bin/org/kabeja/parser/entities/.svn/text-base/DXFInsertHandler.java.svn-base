...
public class DXFInsertHandler extends AbstractEntityHandler {
    public static final String ENTITY_ATTRIB = "ATTRIB";
    ...
    private boolean parse_attrib = false;
    private DXFInsert insert;
    private DXFAttribHandler Hattrib;
    ...
    public void parseGroup(int groupCode, DXFValue value) {
        if ((groupCode == END_SEQUENCE_CODE) ||
        	...
        	Hattrib.endDXFEntity();
        	insert.addAttrib((DXFAttrib)Hattrib.text);
            follow = false;

            return;
        }

        if (groupCode == DXFEntitiesSectionHandler.ENTITY_START){

	        if (ENTITY_ATTRIB.equals(value.getValue())) {
	            ...
	            if (parse_attrib) {
	            	...
	            	parse_attrib = true;
		        	Hattrib = new DXFAttribHandler(); 
	            }
	            //new and clear
	            Hattrib.startDXFEntity();
	        	((DXFAttrib)Hattrib.text).setDXFDocument(doc);
	        }
	        return;
        }

        if (parse_attrib == true){
        	Hattrib.parseGroup(groupCode, value);
