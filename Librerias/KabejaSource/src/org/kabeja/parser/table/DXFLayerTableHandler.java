...
public class DXFLayerTableHandler extends AbstractTableHandler {
     ...
     *      java.lang.String)     groupCode: 2,62,6,70,370,390
     */
    public void parseGroup(int groupCode, DXFValue value) {
        switch (groupCode) {
        case GROUPCODE_LAYER_NAME: //2
            layer.setName(value.getValue());

            break;

        case GROUPCODE_LAYER_COLORNUMBER: //62
            layer.setColor(value.getIntegerValue());

            break;

        case GROUPCODE_LAYER_LINETYPE: //6
            layer.setLineType(value.getValue());

            break;

        case DXFConstants.GROUPCODE_STANDARD_FLAGS: //70
            layer.setFlags(value.getIntegerValue());

            break;

        case GROUPCODE_LAYER_LINEWEIGHT: //370
            layer.setLineWeight(value.getIntegerValue());

            break;

        case GROUPCODE_LAYER_PLOTSTYLENAME: //390
