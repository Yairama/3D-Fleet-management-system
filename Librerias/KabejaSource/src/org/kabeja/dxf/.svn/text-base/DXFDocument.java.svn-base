...
public class DXFDocument {
    ...
    public void addDXFLayer(DXFLayer layer) {
        layer.setDXFDocument(this); //将layer中的doc指针指向本doc
        layers.put(layer.getName(), layer); //将当前layer加入到本doc中对应的layers hash表中
    }
    ...
    public void addDXFEntity(DXFEntity entity) {
        entity.setDXFDocument(this);  //将entity中的doc指针设为本doc

        DXFLayer layer = this.getDXFLayer(entity.getLayerName()); //取得当前entity中的layer信息
        layer.addDXFEntity(entity);//在本doc中的对应layers的hash表中的对应layer的hash表中的对应entity关键字连接上加上此entity
    }

    public void addDXFBlock(DXFBlock block) {
        block.setDXFDocument(this); //将block中的doc指针指向本doc
